...
Container entrypoint for running PathingAPI on Linux.

## Build

```bash
docker build -f PathingAPI/Dockerfile -t pathingapi-linux .
```

The Dockerfile builds StormLib from source inside the image so MPQ archives can be read on Linux (`libstorm.so` is copied into the published output).
